...
# set initial values
	...
	# set graph labels
	$opt[$KEY]     = "--vertical-label $label -l 0 $upper --title \"$hostname / $servicedesc\" ";
	# Graph Definitions
	$def[$KEY]     = rrd::def( "var1", $VAL['RRDFILE'], $VAL['DS'], "AVERAGE" ); 
	# "normalize" graph values
	$def[$KEY]    .= rrd::cdef( "v_n","var1,$divis,/");
	#$def[$KEY]    .= rrd::area( "v_n", "#c6c6c6",  $ds_name[$KEY] );
	$def[$KEY]    .= rrd::area( "v_n", "#BAF4D8",  $ds_name[$KEY] );
	...
	if ($VAL['MAX'] != "") {
		...
		$def[$KEY] .= rrd::hrule( $max[1], "#003300", "Size of Pool  $max[0] \\n");
	}
